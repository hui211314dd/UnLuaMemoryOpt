...
{
    size_t TotalMemory;
    TMap<lua_State*, MemoryDetail> MemoryRecord;
        ...
        : bStarted(false)
    {
        ...
        L = lua_newstate(GetLuaAllocator(), &StateInfo);
    ...
    bool FLuaEnv::GetMemInfo(lua_State* L, size_t size, LuaMemInfo& info)
    {
        if (!L) return false;
    ...
    */               
    ...
    MemoryDetail* FLuaEnv::TryGetMemoryRecord(lua_State* LS)
    {
    ...
    void FLuaEnv::AddRecord(lua_State* LS, void* ptr, size_t size, LuaMemInfo& memInfo)
    {
            return;
            
        if (!LS) return;

        UE_LOG(LogUnLua, Log, TEXT("alloc memory %d from %s lineNumber: %d, TotalMemory: %d"), size, (*memInfo.hint), memInfo.lineNumber, TotalMemory + size);
    ...
    void FLuaEnv::RemoveRecord(lua_State* LS, void* ptr, size_t osize)
    {
            return;
        ...
        if (memInfoPtr)
        {
            ...
            UE_LOG(LogUnLua, Log, TEXT("free memory  %d from %s linenumber: %d, TotalMemory: %d"), osize,  (*memInfoPtr->hint), memInfoPtr->lineNumber, TotalMemory - osize);
    ...
    {
        LuaStateInfo* StateInfo = (LuaStateInfo*)ud;
        lua_State*LS = StateInfo->IsValid ? StateInfo->L : nullptr;
